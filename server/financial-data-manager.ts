...
export class FinancialDataManager {
  ...
  /**
   * Get employee summary data for analytics
   * Returns aggregated employee financial data for given date range
   */
  static async getEmployeeSummary(fromDate?: string, toDate?: string) {
    try {
      console.log(`Getting employee summary from ${fromDate || 'all'} to ${toDate || 'all'}`);
      
      // Query the doctor_performance table
      let query = db.select({
        id: doctorPerformance.id,
        month: doctorPerformance.month,
        revenue: doctorPerformance.revenue,
        expense: doctorPerformance.expense,
        netIncome: doctorPerformance.netIncome,
        marginPct: doctorPerformance.marginPct,
      })
      ...
      if (fromDate) {
        query = query.where(sql`${doctorPerformance.month} >= ${fromDate}`);
      }
      
      if (toDate) {
        query = query.where(sql`${doctorPerformance.month} <= ${toDate}`);
      }
      
      const results = await query;
      
      // Aggregate results by month
      const monthlyData = results.reduce((acc, row) => {
        ...
        acc[month].revenue += row.revenue || 0;
        acc[month].expense += row.expense || 0;
        acc[month].net += row.netIncome || 0;
        
        return acc;
      }, {} as Record<string, { month: string; revenue: number; expense: number; net: number; }>);
  ...
  /**
   * Get employee detail data for analytics
   * Returns detailed financial data for a specific employee for given date range
   */
  static async getEmployeeDetail(employeeId: string, fromDate?: string, toDate?: string) {
    try {
      console.log(`Getting employee detail for ${employeeId} from ${fromDate || 'all'} to ${toDate || 'all'}`);
      
      // Query the doctor_performance table
      let query = db.select({
        id: doctorPerformance.id,
        month: doctorPerformance.month,
        doctorName: doctorPerformance.doctorName,
        revenue: doctorPerformance.revenue,
        expense: doctorPerformance.expense,
        netIncome: doctorPerformance.netIncome,
        marginPct: doctorPerformance.marginPct,
      ...
      .from(doctorPerformance)
      .where(eq(doctorPerformance.doctorName, employeeId))
      .orderBy(doctorPerformance.month);
  ...
  /**
   * Get a list of employees for the dropdown
   */
  static async getEmployeeList() {
    try {
      // Get distinct employee names
      const query = db.selectDistinct({ 
        name: doctorPerformance.doctorName 
      })
      .from(doctorPerformance)
      .orderBy(doctorPerformance.doctorName);
  ...
  /**
   * Get business summary data for analytics
   * Returns aggregated business financial data for given date range
   */
  static async getBusinessSummary(fromDate?: string, toDate?: string) {
    try {
      console.log(`Getting business summary from ${fromDate || 'all'} to ${toDate || 'all'}`);
      
      // Query the department_performance table
      let query = db.select({
        id: departmentPerformance.id,
        month: departmentPerformance.month,
        revenue: departmentPerformance.revenue,
        expense: departmentPerformance.expense,
        netIncome: departmentPerformance.netIncome,
        marginPct: departmentPerformance.marginPct,
      })
      ...
      // Apply date range filters if provided
      if (fromDate) {
        query = query.where(sql`${departmentPerformance.month} >= ${fromDate}`);
      }
      
      if (toDate) {
        query = query.where(sql`${departmentPerformance.month} <= ${toDate}`);
      }
  ...
  /**
   * Get business detail data for analytics
   * Returns detailed financial data for a specific business line for given date range
   */
  static async getBusinessDetail(businessId: string, fromDate?: string, toDate?: string) {
    try {
      console.log(`Getting business detail for ${businessId} from ${fromDate || 'all'} to ${toDate || 'all'}`);
      
      // Query the department_performance table
      let query = db.select({
        id: departmentPerformance.id,
        month: departmentPerformance.month,
        departmentName: departmentPerformance.departmentName,
        revenue: departmentPerformance.revenue,
        expense: departmentPerformance.expense,
        netIncome: departmentPerformance.netIncome,
        marginPct: departmentPerformance.marginPct,
      ...
      .from(departmentPerformance)
      .where(eq(departmentPerformance.departmentName, businessId))
      .orderBy(departmentPerformance.month);
  ...
  /**
   * Get a list of business lines for the dropdown
   */
  static async getBusinessList() {
    try {
      // Get distinct business names
      const query = db.selectDistinct({ 
        name: departmentPerformance.departmentName 
      })
      .from(departmentPerformance)
      .orderBy(departmentPerformance.departmentName);
