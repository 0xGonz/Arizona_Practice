...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Employee Analytics
  app.get("/api/analytics/employee/list", async (req, res) => {
    try {
      const employees = await FinancialDataManager.getEmployeeList();
  ...
  app.get("/api/analytics/employee/summary", async (req, res) => {
    try {
      const { from, to } = req.query;
      const fromDate = from as string | undefined;
      const toDate = to as string | undefined;
      
      const data = await FinancialDataManager.getEmployeeSummary(fromDate, toDate);
  ...
  app.get("/api/analytics/employee/detail", async (req, res) => {
    try {
      ...
      const data = await FinancialDataManager.getEmployeeDetail(employeeId, fromDate, toDate);
      res.status(200).json(data);
    } catch (error) {
  ...
  // Business Analytics
  app.get("/api/analytics/business/list", async (req, res) => {
    try {
      const businesses = await FinancialDataManager.getBusinessList();
  ...
  app.get("/api/analytics/business/summary", async (req, res) => {
    try {
      const { from, to } = req.query;
      const fromDate = from as string | undefined;
      const toDate = to as string | undefined;
      
      const data = await FinancialDataManager.getBusinessSummary(fromDate, toDate);
      res.status(200).json(data);
    } catch (error) {
  ...
  app.get("/api/analytics/business/detail", async (req, res) => {
    try {
      ...
      const data = await FinancialDataManager.getBusinessDetail(businessId, fromDate, toDate);
      res.status(200).json(data);
    } catch (error) {
